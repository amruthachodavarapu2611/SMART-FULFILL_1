...
def dashboard():
    ...
    cursor.execute("""
        ...
        FROM orders
        WHERE status IN
        ('PENDING','FULLY_ALLOCATED','PARTIALLY_ALLOCATED',
         'PICKING','PACKING','QUALITY_CHECK')
    """)
    ...
    fulfillment_rate = int(
        (fulfilled / max(total_orders, 1)) * 100
    )
...
def place_order():
    ...
    # Generate ID BEFORE decision engine
    cursor.execute(
    ...
    priority_result = prioritize_order(
        order_data
    )
    ...
    priority_level = priority_result[
        "priority_level"
    ]
    ...
    flash(
        ...
        f"with priority score {priority_score:.1f}"
    )
...
def allocate(order_id):
    ...
    flash(
        f"Order {order_id} allocated: {decision}"
    )
...
def batch_allocate():
    ...
    flash(
        f"Batch allocation completed for "
...
def complete_picking(order_id):
    ...
    flash(
        f"Picking completed for {order_id}"
    )
...
@app.route(
    "/packing/<order_id>/start",
    methods=["POST"]
)
def start_packing(order_id):

    packer_name = request.form.get(
        "packer_name",
        "Unassigned"
    )
    ...
    flash(
        f"Packing started for {order_id}"
    )
...
def complete_packing(order_id):
    ...
    flash(
        f"Packing completed for {order_id}"
    )
...
def quality_check():
    ...
    # IMPORTANT FIX:
    # QC page must read QUALITY_CHECK orders
    cursor.execute("""
        SELECT
            o.*,
            p.name AS product_name
...
@app.route(
    "/quality-check/<order_id>/start",
    methods=["POST"]
)
def start_qc(order_id):

    inspector = request.form.get(
        "inspector",
        "Unassigned"
    )
    ...
    if passed:
        ...
        if order:
            ...
            """, (
                ...
                order["product_id"],
                (
                    f"QC inspection found "
                    f"{defects} defects: {notes}"
                )
            ))
            ...
            flash(
                f"QC failed for {order_id} - "
                f"Exception {exc_id} created"
            )
...
def analytics():
    ...
    cursor.execute(
        "SELECT SUM(current_stock) FROM products"
    )
    total_stock = (
    ...
    cursor.execute(
        "SELECT SUM(reserved_stock) FROM products"
    )
    reserved_stock = (
        cursor.fetchone()[0] or 0
    )
    ...
    cursor.execute(
        "SELECT SUM(damaged_stock) FROM products"
    )
    damaged_stock = (
        cursor.fetchone()[0] or 0
    )
